...
export default new (class {
  ...
  aes_encode(data: any, key = '', iv = '') {
    if (!data) return ''
    const clearEncoding: crypto.Encoding = 'utf8'
    const cipherEncoding: crypto.BinaryToTextEncoding = 'base64'
    const cipherChunks = [] as string[]
  ...
  session_decode(str: string) {
    ...
    if (expire * 1000 < Date.now()) return null
